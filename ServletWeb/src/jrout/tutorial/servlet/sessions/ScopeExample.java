...
public class ScopeExample extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		Date createTime = new Date(session.getCreationTime()); // 9:11 pm
		// Get last access time of this web page.
		Date lastAccessTime = 
				new Date(session.getLastAccessedTime());  // 9:15 9:20

		String title ="";
		Integer visitCount = new Integer(0);
		Integer requestCount = new Integer(0);
		String visitCountKey = new String("visitCount");
		String fname = request.getParameter("fname");//new String("userID");
		String lname = request.getParameter("lname"); //new String("ABCD");
		...
		if (session.isNew()){
			...
			lname = (String)session.getAttribute("name");
